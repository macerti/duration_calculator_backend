...
### FEAT-001 — 2026-09-01: Synthèse tabs for per-site programmes and consolidated client audit programme

- **Type**: UX / reporting feature request.
- **Status**: REQUESTED — not implemented.
- **Area**: **Synthèse**.

#### Objective
The Synthèse should make the audit programme understandable at two levels: **per site** and **client-wide consolidated**.

#### Required UX
- Use **tabs** in Synthèse to switch between the different sites.
- Each site tab must display that site's complete audit programme and its relevant duration details.
- Add a dedicated **global/consolidated tab** that combines all applicable sites and presents the **final audit programme for the client, sites combined**.
- The consolidated view must not simply replace the individual site views: both levels must remain accessible.

#### Expected information hierarchy
1. **Global / Client — Sites confondus**
   - Final consolidated audit programme for the client.
   - Combined durations across the applicable sites.
   - Clear breakdown sufficient to understand how the consolidated programme is constructed.
2. **Siège**
   - Programme and durations specific to the headquarters, where applicable.
3. **Site 1, Site 2, ...**
   - Programme and durations specific to each site.

The exact tab labels can follow the application's existing terminology, but the distinction between **individual site programme** and **client-wide consolidated programme** must be unambiguous.

#### Relationship with existing requirements
- This extends BUG-027 section 2: Synthèse must provide useful annual duration information per site and per standard instead of relying on a generic global **Total jour à auditer** figure.
- This must work together with BUG-025's multi-standard Synthèse behavior: selecting a standard must update the programme/details for the currently selected site/global context without leaking state between tabs.
- The global consolidated tab must combine the underlying calculated results; it must not double-count sites or standards.

#### Acceptance criteria
- A multi-site calculation produces one Synthèse tab per applicable site plus one dedicated consolidated/client tab.
- Switching site tabs changes all displayed site-specific programme and duration information.
- Switching to the consolidated tab displays the final client programme with all sites combined.
- Multi-standard sites retain their standard-specific breakdown inside the appropriate site/global context.
- The consolidated totals reconcile mathematically with the underlying per-site results according to the existing calculation/synergy rules.
- Existing calculation formulas and engine behavior are unchanged unless a separate, explicitly logged calculation defect is identified.
