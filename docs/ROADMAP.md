...
#### 9. Local email/password accounts + role-based access control (IN PROGRESS — data layer done & verified 2026-09-04, API/mailer/frontend not started)
- **Category:** Security & Identity
- **Status as of 2026-09-04 (twenty-fourth session): schema + backend data layer done and verified against a real database; nothing wired into `api/index.php` yet; no mailer; no frontend work at all.** Full detail, design rationale, and exact hand-off steps: `docs/DEV_STATUS.md`, 2026-09-04 (twenty-fourth session) entry — read that before touching this item.
...
- **Done this session**: migration `002_add_auth_and_rbac.sql` (users/roles/permissions/role_permissions/user_identities/email_verification_tokens/password_reset_tokens/rate_limits, seeded with 3 default roles and 6 starting permissions); `userRepo.php`/`roleRepo.php`/`permissionRepo.php`/`rateLimiter.php`, all exercised against a real database (25 passing assertions) before commit. This satisfies FEAT-002's "explicit account-linking policy" requirement referenced below — see the account-linking rules in `userRepo.php`'s `resolveSsoUser()` and this session's DEV_STATUS.md entry for exactly what they do.
- **Not done yet, in priority order for the next session**: (a) `Guard.php` (auth/permission/CSRF checks) and gating the pre-existing `/clients`/`/cases` endpoints with it — closing `SECURITY.md`'s own "Todo #1, the single biggest gap" is part of this item, not separate; (b) `Mailer.php` — needs Mahdi to supply SMTP credentials for `info@macerti.com` (host/port/username/password), or an explicit go-ahead to ship with a dev-only `log` driver first; (c) the `/auth/*` and `/admin/*` HTTP routes themselves; (d) all frontend work — `useAuth.ts`, `LoginScreen.tsx` (add local email/password alongside existing Microsoft SSO), and new Register/ForgotPassword/ResetPassword/Profile/AdminUsers/AdminRoles screens. None of this is designed-but-undone — the design is finished (see DEV_STATUS.md entry); what's left is implementation.
