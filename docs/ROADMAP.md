...
- **Status:** Immediate incident CONFIRMED RESOLVED 2026-09-04 — Mahdi applied the migration manually and confirmed Microsoft sign-in works again (`docs/BUGLOG.md` BUG-045). **The underlying automation gap is still open and is now the top-priority item in this list** — Mahdi explicitly confirmed he expected pushes to update the database automatically, which is not yet true and will bite again on the next migration if left unbuilt.
...
#### 9. Local email/password accounts + role-based access control (IN PROGRESS — backend done & verified end-to-end 2026-09-04, only the frontend remains)
- **Category:** Security & Identity
- **Status as of 2026-09-04 (twenty-sixth session): the entire backend is real, wired into `api/index.php`, and verified against a live PHP server + real MariaDB (42/42 HTTP tests, including CSRF enforcement, SSO account-linking, and the pre-existing `/clients`/`/cases` routes now correctly requiring login). BUG-044 (CI red on `f45129d`) is now fixed and locally re-verified end-to-end — see `docs/BUGLOG.md`. Confirm the real Actions run on this session's push before treating it as fully closed. Nothing in the frontend has changed yet — that's the entire remaining scope.** Full detail: `docs/DEV_STATUS.md`, 2026-09-04 (twenty-fourth, twenty-fifth, and twenty-sixth session) entries — read all three before touching this item.
...
- **Not done yet — this is now the entire remaining scope**: the frontend. `useAuth.ts`, `LoginScreen.tsx` (add local email/password alongside existing Microsoft SSO), and new Register/ForgotPassword/ResetPassword/Profile/AdminUsers/AdminRoles screens. Every backend endpoint they need already exists and is tested. Also still open: real SMTP credentials for `info@macerti.com` (the `smtp` mailer driver is written but untested against an actual server — `log` driver is what's been verified), and extending the new CSRF-token mechanism to the pre-existing `/clients`/`/cases` mutating routes (deliberately scoped out this session, not forgotten).
