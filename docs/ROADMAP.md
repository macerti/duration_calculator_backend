...
- **Category:** Security & Identity
- **Status as of 2026-09-04 (twenty-fifth session): the entire backend is real, wired into `api/index.php`, and verified against a live PHP server + real MariaDB (42/42 HTTP tests, including CSRF enforcement, SSO account-linking, and the pre-existing `/clients`/`/cases` routes now correctly requiring login). Nothing in the frontend has changed yet — that's the entire remaining scope.** Full detail: `docs/DEV_STATUS.md`, 2026-09-04 (twenty-fourth and twenty-fifth session) entries — read both before touching this item.
