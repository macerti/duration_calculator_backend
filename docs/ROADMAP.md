...
#### 8. FEAT-005 — Automated database schema migration on push (IMPLEMENTED, PUSHED, CI FAILED — currently being debugged, NOT working end-to-end yet)
- **Category:** Infrastructure / DevOps
- **Status as of 2026-09-04 (twenty-second session): CI-BLOCKED, under active fix.** Implemented and pushed by the twenty-first session (commit `51abb8c`) — idempotent SQL migration framework (`src/backend/db/Migrations.php`, `db/migrate.php`, `db/migrations/001_initial_schema.sql`, `db/migrations/README.md`), wired into `.github/workflows/build-test-publish.yml`'s "Run database migrations" step. **That CI run failed on exactly that step**, which meant the push never reached the deployment repo at all (every later step — tests, frontend build, publish — was skipped as a consequence). The twenty-second session found and fixed four layered bugs in the migration runner (config loading, DDL-vs-transaction handling, an unclosed statement cursor, and an unsafe statement-execution call in the framework's own idempotent-guard pattern) — three are independently verified fixed; the fourth (the deepest one) has a fix applied but **not yet re-verified end-to-end**. Full detail, exact reproduction steps, and the precise next action: `docs/BUGLOG.md` BUG-040 through BUG-043 (read BUG-043's "Not done / open" section first).
- **Request as originally given (2026-09-03, by Mahdi):** database migration so that if there is an update in tables, it's enough to push so that the GitHub Action updates the database structure if needed.
- **Do not mark this feature as done/working until**: (a) the local end-to-end verification sequence in BUG-043 passes (fresh DB → migrate twice for idempotence → seed → full regression suite), and (b) an actual GitHub Actions run on a fresh push is confirmed green — local success alone was not sufficient evidence for the twenty-first session's original push, so it shouldn't be treated as sufficient now either.
- **Priority note (2026-09-03, twentieth session), still standing**: this remains first in line among the two items below — do local-password account creation (#9) only after this one is actually confirmed working, so that feature's own schema changes don't need a manual DB update either.
