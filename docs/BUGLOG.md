...
- Real Apache + `.htaccess` topology test — still never performed in any session. Lower-stakes now than before this fix (the new code no longer depends on Apache's `SCRIPT_NAME` behavior matching any particular assumption), but still the right thing to close out given `.htaccess`-based deny rules (`.sql`/`.csv`/`.bak` blocking) and the `RewriteRule ^ index.php` dispatch itself have *also* never been verified against a real Apache instance, only reasoned about.
