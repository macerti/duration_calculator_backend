...
**Status: PARTIALLY FIXED 2026-09-02 (fifteenth session). One confirmed frontend bug fixed and verified. The underlying reason sign-in doesn't actually complete is NOT yet confirmed — needs one piece of live evidence only Mahdi/a browser can provide (see "Next step" below). Do not close this bug until that's checked.**
...
**Status: PARTIALLY FIXED 2026-09-03 (sixteenth session). The discard bug is fixed and verified. The underlying reason Microsoft itself is rejecting the request is still NOT identified — needs one piece of live evidence only Mahdi/a browser can now provide, and this time it will be the actual AADSTS code, not a bare generic string.**
...
**Next step (same shape as BUG-037's, now with a sharper target)**: with this session's fix live, retry the Microsoft sign-in once and report back the **exact banner text** — it will now read `invalid_request: <the real AADSTS explanation>` instead of a bare code. That AADSTS number is very likely to name the fix outright (e.g. a specific platform-type mismatch to correct in Azure Portal, not code). Do not re-derive today's reasoning from scratch; the two informed-but-unconfirmed candidates above are the starting point, not the answer.

**Dependency / hand-off**: this session's fix (commit below) is source-only per the mandatory source/deployment separation rule — not yet published to `macerti/duration_calculator`. CI publishes on push to `main`; confirm the publish workflow completed before asking Mahdi to retry, or the banner will still show the old bare code.
