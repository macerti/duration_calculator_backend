...
- **CI-confirmed on real GitHub Actions**: not yet — this session's changes are about to be pushed for the first time as this entry is written; see the push/PR evidence a following update (same session or next) should record once the resulting Actions run is observed, the same discipline every prior FEAT-005-adjacent session followed (don't claim "CI green" until an actual run has been watched, not assumed).
...
**NOT DONE / open, in priority order**:
1. **Observe the real GitHub Actions run this push triggers** (both repos) and confirm green end-to-end, including the new `deploy.yml` step actually reaching the live host — CI-only confirmation was exactly what let BUG-045 slip through once already; don't repeat that mistake by assuming success from local sandbox results alone.
2. **Confirm with Mahdi that the one manual step (adding `migration_secret` to production `config.php`) has been done.** Until then, the new deploy step will fail loudly (by design) rather than silently doing nothing — that failure is expected and is not a new bug if seen once, but should not still be happening on the second push after this one.
