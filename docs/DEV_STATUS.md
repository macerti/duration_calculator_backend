...
4. USER FEEDBACK / ACCEPTANCE GATE. After the items above, pause normal feature development and perform real browser/mobile/user testing. Feed the results back into the logs to definitively close, reopen, or change the relevant bugs/features. **IN PROGRESS — Microsoft SSO feedback received 2026-09-03 (sixteenth session): `⚠ invalid_request` banner reported after account selection. Root-caused to a real code bug (see BUG-038) — the diagnostic detail was being discarded before it could ever reach anyone, not a dead end. Fixed and pushed; still waiting on the next retry's banner text to identify the actual Microsoft-side rejection reason. Do not treat this as closed, and do not advance to items 5/6 for the SSO feature until BUG-038 is resolved — everything else in this gate (the calculation engine, clients/cases CRUD) is untouched by this and can still be tested independently.**
