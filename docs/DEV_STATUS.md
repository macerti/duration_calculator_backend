...
- **BUG-038 (Microsoft SSO rejected with `invalid_request`) — ROOT CAUSE CONFIRMED 2026-09-03 (seventeenth session): AADSTS9002325, redirect URI registered as "Single-page application" instead of "Web" in Azure Portal.** Fix is a Portal configuration change, not code — cannot be applied from this sandbox. Blocks FEAT-002 (SSO) only; no other part of the app is affected. See `docs/BUGLOG.md` BUG-038 for the exact steps and evidence.
...
5. **Authentication & SSO**: Microsoft Entra ID & Google Account sign-in with PHP session backend. **BUG-036 (outage) fixed and confirmed reaching Microsoft's login. BUG-038 (open, root cause confirmed): Azure Portal redirect URI is registered under the wrong platform type (SPA instead of Web) — AADSTS9002325. This is a Portal config fix for Mahdi, not further code work; see `docs/BUGLOG.md` BUG-038 for exact steps. Do not attempt a code-side workaround — the app's confidential/server-side flow is already correct as written.**
