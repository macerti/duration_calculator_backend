...
- **BUG-039 (Microsoft SSO now fails post-consent with `callback_failed`) — OPEN, eighteenth session (2026-09-03). Our own code's catch-all for a failed server-side token exchange; the real exception detail was being discarded the same way BUG-038's provider-error branch used to. FIXED (visibility only): the detail now reaches the banner via `auth_error_description`, same mechanism BUG-038 built. The actual reason the token exchange fails is NOT yet known** — leading unconfirmed hypothesis (carried over from BUG-037's original candidate list, never ruled out): wrong Azure client secret (Secret ID vs Secret Value). **Do not guess-fix — wait for the next retry's banner text.** Blocks FEAT-002 (SSO) only; no other part of the app is affected. See `docs/BUGLOG.md` BUG-039 for the full evidence trail and remaining candidates.
...
5. **Authentication & SSO**: Microsoft Entra ID & Google Account sign-in with PHP session backend. **BUG-036 (outage) fixed. BUG-038 (Azure Portal SPA-vs-Web platform type) fixed by Mahdi. BUG-039 (open, eighteenth session): a NEW error, `callback_failed`, appeared once BUG-038 was fixed — our own server-side token exchange is now failing for an unknown reason (Microsoft's `/authorize` step itself succeeds). The exception detail is now surfaced to the banner (see `docs/BUGLOG.md` BUG-039); the underlying cause is not yet known — do not guess-fix, wait for the next retry's `auth_error_description` text.**
